-- Extend place_order() to also persist tax_total (column added in 0009).
...
begin
  insert into orders (
    ...
    subtotal, shipping_total, discount_total, tax_total, total, notes, coupon_code,
  ...
  if p_order->>'coupon_code' is not null then
    update coupons set times_used = times_used + 1 where code = p_order->>'coupon_code';
  end if;
